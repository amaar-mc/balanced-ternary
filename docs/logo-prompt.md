...
Subject: the balanced ternary number system with digits minus-one, zero, plus-one. A horizontal row of small square cells, each cell holding one of three symbols: a minus bar, an empty dot, or a plus, some cells in the blue accent; above the row a simple balance beam or scale tipping to show the three-state symmetry around zero. Clean number-system schematic, no letters or numerals.
